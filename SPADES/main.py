...
def main():
    ...
    while gamelib.is_alive():
        ...
        if spades.juego_terminado(juego):
            ...
            juego = spades.crear_juego(juego_nuevo())
            continue #Asi muestro en pantalla las cartas nuevas antes de apostar

        if not juego.ronda.vuelta.numero_vuelta: #Si la vuelta es la 0
            if not spades.todos_apostaron(juego):
        ...
        if len(juego.ronda.vuelta.cartas_puestas) == len(juego.jugadores) and not spades.juego_terminado(juego): #Si todos jugaron sus cartas y la ronda no es la ultima
